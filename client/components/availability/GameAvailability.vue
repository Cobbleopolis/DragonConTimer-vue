<template>
    <div>
        ...
        <b-card-text>{{$t('availability.games.total', {count: game.count})}}</b-card-text>
        <b-card-text v-if="copiesAvailable > 0">{{$t('availability.games.available', {count: copiesAvailable})}}</b-card-text>
        <b-card-text v-else>{{$t('availability.games.availableIn', {time: timeToNextGame})}}</b-card-text>
