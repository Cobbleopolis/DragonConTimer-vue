<template>
    <b-card :header="console.name" no-body class="mb-3">
        <b-card-body>
            <b-card-text v-if="stationsWithConsoleStatus(this.StationStatus.DEFAULT).length > 0">
                {{
                    $t('availability.consoles.available', {
                        count: stationsWithConsoleStatus(this.StationStatus.DEFAULT).length,
                        total: stationsWithConsole.length
                    })
                }}
            </b-card-text>
            <b-card-text v-else>{{ $t('availability.consoles.availableIn', {time: timeToNextConsole}) }}</b-card-text>
            <b-card-text v-if="outOfTimeStations.length > 0">
                {{ $t('availability.consoles.needToBeKicked', {stations: outOfTimeStations}) }}
            </b-card-text>
        </b-card-body>
